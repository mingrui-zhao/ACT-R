...
def extract_camera_params(npz_file_path: str) -> tuple[np.ndarray, float]:
    """Extract elevations and radius from camera parameter file.
    
    Returns:
        Tuple[np.ndarray, float]: Array of 21 elevations in degrees, radius
    """
    cam_data = np.load(npz_file_path)
    elevations = []
    
    # Get elevation for each view
    for i in range(21):
        world_mat = cam_data[f'world_mat_{i}']
        el, distance = decompose_world_mat(world_mat)
        elevations.append(el)
    
    return np.array(elevations), distance
...
for idx, image_file in enumerate(input_files):
    ...
    image = np.array(Image.open(os.path.join(image_path, f'{name}.png'))).astype(np.float32) / 255.0
    image = torch.from_numpy(image).permute(2, 0, 1).contiguous().float()     # (3, 960, 640)
    image = rearrange(image, 'c (n h) (m w) -> (n m) c h w', n=3, m=2)        # (6, 3, 320, 320)
    outputs.append({'name': name, 'images': image})
...
for idx, sample in enumerate(outputs):
    ...
    camera_data_path = os.path.join(camera_path, name+'.npz')
    elevations, radius = extract_camera_params(camera_data_path)
    # import pdb; pdb.set_trace()
    input_cameras = get_actr_input_cameras(elevations, radius=radius).cuda()
    if args.view == 4:
